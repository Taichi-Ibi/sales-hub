...
export const DEAL_ENTRIES: DealEntry[] = [
  {
    ...
    updates: [
      { at: '6/10 06:00', kind: '定期更新', summary: 'スナップショット生成（フェーズ: 提案 → 契約）' },
      {
        at: '6/9 18:00',
        kind: '取込',
        summary: 'メール「契約条件ご承諾のご連絡」を取込（条件承諾・契約書ドラフトへ）',
        source: { label: 'メール 6/9「契約条件ご承諾のご連絡」', inboxItemId: 'in12' },
      },
      { at: '6/3 16:00', kind: '取込', summary: '議事録「B商事 定例」を取込（下期1割増・価格据え置きで最終調整）' },
    ],
...
export const SNAPSHOTS: WikiSnapshot[] = [
  // ── B商事: 提案 → 契約（今日の最重要イベント）──
  {
    ...
    markdown: `## 概要
- 年間売買基本契約の更新交渉。価格・数量・支払いサイトの条件見直しが主論点。

## 現在の状況
- 下期発注量1割増・価格据え置きを軸に、6月中の最終合意を目指している。 [tr:tr-b1]
- 6/17の打ち合わせで最終合意を予定。 [tr:tr-b1]

## 経緯
- 5/28 先方より下期発注量を1割増やしたい意向（価格据え置きが条件）。 [tr:tr-b1]`,
  },
  {
    ...
    markdown: `## 概要
- 年間売買基本契約の更新交渉。先方が更新条件を承諾し、契約書ドラフトの段階に入った。

## 現在の状況
- 6/9 先方より更新条件（下期1割増・価格据え置き）の承諾連絡を受領。契約フェーズへ移行。 [tr:tr-b2]
- 契約書ドラフトの準備に入る。6/17の打ち合わせは条文の最終確認の場に変更。 [tr:tr-b2]

## 経緯
- 6/9 条件承諾のメールを受領（年間取引額は2,640万円規模へ）。 [tr:tr-b2]
- 5/28 先方より下期発注量を1割増やしたい意向（価格据え置きが条件）。 [tr:tr-b1]`,
  },
  ...
  // ── 北斗電装: 変化なし（6:00時点）。in02 のゲート通過で実行時に動く ──
  {
    ...
    markdown: `## 概要
- 継続売買契約 〔契約番号①〕 の解約条項（通知期間90日）について、先方調達部門で不満が強まっている。
...
- 6/5 社内Slackで先方調達部門の不満の兆候を共有。 [tr:tr-h1]`,
  ...
  // ── G産業: 商談フェーズが14日間動いていない（滞留）──
  {
    ...
    markdown: `## 概要
- G産業への新規製品提案。製品資料の送付と先方社内検討会への資料提供を通じて受注獲得を目指す。

## 現在の状況
- 約束済みの製品資料送付が未対応のまま。6/12の先方社内検討会が迫っている。 [tr:tr-g2]

## 経緯
- 5/27 初回商談。先方は社内検討会（6/12）に向けて資料提供を希望。 [tr:tr-g1]`,
  },
  {
...
- 5/27 初回商談。先方は社内検討会（6/12）に向けて資料提供を希望。 [tr:tr-g1]`,
  ...
  // ── 湊精機: 確度 70 → 85（好転）──
  {
    ...
    meta: {
      deal_id: 'd-minato',
      deal_name: '保守契約 更新（HSK-2025-007）',
      phase: '契約',
      confidence: 70,
      amount: '360万円/年',
    ...
    markdown: `## 概要
- 工作機械保守サービス年間契約の更新。現行条件での継続更新が目標。
