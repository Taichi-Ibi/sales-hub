...
raw（受信箱）      Slack/メール/議事録の原文。不変。
  ↓ 前処理        ローカルで自動マスク・案件判定（データは外に出ない）
  ↓ HITL承認      ★ AIへの送信は全件、人の承認が必要（セキュリティポリシー）
...
設計原則は **「AIが処理し、境界は人が握る（HITL）」**:
ローカル前処理（分かち書き・ルール/辞書マスク・案件自動判定）が下ごしらえを済ませ、
人は **AI送信前レビュー** で確認・補正して承認する。これがセキュリティ境界。
人の仕事は 1) 「今日」のタスクをこなす、2) レビューを承認する、の2つ。
...
```
src/
  data/inbox.ts          # raw層: 受信箱のサンプルデータ（原文・自動マスク・承認前警告）
  ...
  pages/
    ...
    Inbox.tsx            # 受信箱（AI送信前レビュー＋予定＋処理ログ）
    InboxDetail.tsx      # 原文詳細（マスク補正→案件選択→承認してAIに渡す）
...
3. **受信箱** `/inbox` — AI送信前レビュー（HITL）。バッジ=レビュー待ち件数
...
- 受信箱（InboxItem）の status は4値:
  ...
  - `レビュー待ち`: AI送信前レビュー（全件必須のHITLゲート）。ルールが検出した警告は
    `attention` に入る。人がマスク補正・案件選択をして「承認してAIに渡す」
    → 解析（1.5秒シミュレート）→ `処理済み`
  - `処理済み`: 承認→AI解析完了（`processedAt` / `approvedBy` が監査ログ）。
    タスク化（`resultActionId`）または `analysisNote`
  - `アーカイブ`: AIに渡さないと人が判断したもの（戻せる）
- 承認でタスク化すると、該当案件の wiki に「取込」更新が実行時追記される
  （`StoreContext` の `wikiAppends`。`ProjectDetail` が静的 `updates` とマージ表示）

### 受信箱（AI送信前レビュー / HITL）のパイプライン

- 建て付け: 受信 → ローカル前処理（分かち書き＝初期化時に付与・ルール/辞書による自動マスク
  〔連絡先・契約番号はパターン、氏名は `SettingsMasking` の辞書〕・案件の自動判定
  〔メール: ドメイン / 本文一致〕）→ **人の承認（必須）** → AI解析 → タスク化
- セキュリティポリシー: **AIへの送信は全件、人の承認が必要（HITL）**。
  自動化はレビューを速くするための補助であり、ゲートを迂回しない
- レビュー待ちのシードは3種: 警告「未マスクの疑い」（in02）・警告「案件不明」（in10）・
  警告なしで即承認できる例（in05）
- マスキング（伏せる操作）は受信箱のレビュー時のみ。タスク側は復元のみ
- タスク化時、`knownSensitive` のうち未マスクの語は Action の「未マスクの疑い」に引き継ぐ
- wiki に反映してよいのは**人が承認して AI に渡った原文だけ**。未承認アイテムを
