...
- ナビゲーション: `md` 以上は左サイドバー（176px・テキストリンク）、`md` 未満はヘッダー下の
  横並びリンク列に畳む（下部タブは使わない）。項目・件数は `Shell.tsx` の `items` 配列を共有。
- infobox は `md` 未満で右浮動を解除して全幅（index.css のメディアクエリ）。
- 検索ボックスは `sm` 未満で省略。表（wikitable）は狭幅で横スクロール許容。
